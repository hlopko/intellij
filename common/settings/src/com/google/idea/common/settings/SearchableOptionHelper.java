...
public final class SearchableOptionHelper {
   ...
   * SearchableOption#label()}, {@link SearchableOption#searchResult()}, and {@link
   * SearchableOption#tags()}.
   */
  public void registerOption(SearchableOption option) {
    if (option.searchResult().isPresent()) {
      String searchResult = option.searchResult().get();
      registerWords(registrar.getProcessedWordsWithoutStemming(searchResult), searchResult);
      registerWords(option.tags(), searchResult);
      registerWords(
          registrar.getProcessedWordsWithoutStemming(option.label()), /* searchResult= */ null);
    } else {
      registerWords(
