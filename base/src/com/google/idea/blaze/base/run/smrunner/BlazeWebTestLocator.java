...
class BlazeWebTestLocator implements SMTestLocator {
  ...
  public List<Location> getLocation(
    ...
    List<String> components = on(SmRunnerUtils.TEST_NAME_PARTS_SPLITTER).splitToList(path);
    ...
    for (Dependency dependency : wrapperTarget.getDependencies()) {
      ...
      if (BlazeWebTestConfigurationProducer.getWrappedTestSuffixes().stream()
          .noneMatch(label.targetName().toString()::endsWith)) {
        continue;
      }
      ...
      String url = recreateUrl(handler, protocol, label, kind, components);
      if (url == null) {
        continue;
      }
  ...
  private static String recreateUrl(
    ...
    switch (protocol) {
      case SmRunnerUtils.GENERIC_SUITE_PROTOCOL:
        if (components.size() != 2) {
          return null;
        }
        return handler.suiteLocationUrl(label, kind, components.get(1));
      case SmRunnerUtils.GENERIC_TEST_PROTOCOL:
        if (components.size() != 4) {
          return null;
        }
        return handler.testLocationUrl(
            label, kind, components.get(1), components.get(2), components.get(3));
      default:
        return null;
    }
  }
  ...
  private static List<Location> locate(
    ...
    return locator.getLocation(components.get(0), components.get(1), project, scope);
  }
}
